...
def cardapio_cliente(request, slug):
    mesa = get_object_or_404(Mesa, slug=slug)
    categorias = Categoria.objects.annotate(ativos=Count('itemcardapio', filter=Q(itemcardapio__ativo=True))).filter(ativos__gt=0)

    itens_sem_categoria = ItemCardapio.objects.filter(categoria__isnull=True, ativo=True)

    return render(request, 'clientes/cardapio_cliente.html', {'mesa':mesa, 'categorias': categorias, 'itens_sem_categoria': itens_sem_categoria})